#ifndef GRAPHIR_CODEGEN_GRAPHSCHEDULING_H
#define GRAPHIR_CODEGEN_GRAPHSCHEDULING_H
  ...
  LoopTreeNode* GetOrCreateLoopNode(BasicBlock* Header) {
    if(!LoopTree.count(Header)) {
      LoopTree.insert(std::make_pair(
        Header,
        graphir::make_unique<LoopTreeNode>(Header)
      ));
    }
...
public:
  ...
  using schedule_iterator
    = boost::transform_iterator<graphir::unique_ptr_unwrapper<GraphSchedule>,
