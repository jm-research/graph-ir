...
#include "graphir/CodeGen/DLXNodeUtils.h"
...
void GraphSchedule::OnConnectBlock(BasicBlock* Pred, BasicBlock* Succ) {
  if(!DomNodes.count(Pred))
    DomNodes[Pred] = graphir::make_unique<DominatorNode>(Pred);
  if(!DomNodes.count(Succ))
    DomNodes[Succ] = graphir::make_unique<DominatorNode>(Succ);
...
struct GraphSchedule::DomTreeProxy {
  ...
  using block_iterator
    = boost::transform_iterator<graphir::unique_ptr_unwrapper<BasicBlock>,
  ...
  static block_iterator block_begin(GraphSchedule& Schedule) {
    graphir::unique_ptr_unwrapper<BasicBlock> functor;
...
get(boost::vertex_index_t tag, const GraphSchedule::TreeHandle<ProxyT>& g) {
  return graphir::GraphScheduleVertexIdMap(g.getImpl());
}
} // end namespace graphir
...
GraphScheduler::schedule_begin() {
  graphir::unique_ptr_unwrapper<GraphSchedule> Functor;
