...
#define CASE(OC, S)  \
  ...
  switch (N->getOp()) {
    ...
    case IrOpcode::ConstantStr: {
      OS << "ConstStr<" << NodeProperties<IrOpcode::ConstantStr>(N).str(G)
         << ">";
      break;
    }
    case IrOpcode::FunctionStub: {
      auto* FuncStart =
          NodeProperties<IrOpcode::FunctionStub>(N).getFunctionStart(G);
      std::string Name = "N/A";
      if (FuncStart)
    ...
    case IrOpcode::Call: {
      ...
      auto* FuncStart =
          NodeProperties<IrOpcode::FunctionStub>(FuncStub).getFunctionStart(G);
      std::string Name = "N/A";
      if (FuncStart)
        Name = NodeProperties<IrOpcode::Start>(FuncStart).name(G);
      OS << "Call<" << Name << ">";
...
#include "graphir/Graph/DLXOpcodes.def"
    default:
      graphir_unreachable("Opcodes can't be printed");
  }
  return OS;
#undef CASE
#undef STR
